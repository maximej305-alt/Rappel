...
void main() {
  ...
  group('CustomSoundService — fichiers orphelins', () {
    test('fileExists détecte un fichier présent', () {
      expect(CustomSoundService.fileExists(customId(existingFile)), isTrue);
    });

    test('fileExists détecte un fichier manquant', () {
    ...
    test('fallbackSoundId laisse les sons intégrés intacts', () {
      expect(CustomSoundService.fallbackSoundId('alarm'), 'alarm');
      expect(CustomSoundService.fallbackSoundId('chime2'), 'chime2');
    });
  });

  group('NotificationService — jamais de notification silencieuse', () {
    final s = AppStrings.fr;

    test('un son custom manquant utilise le canal et le son par défaut',
        () {
  ...
  group('SoundPreviewService — aperçu orphelin', () {
    ...
    test('stop et dispose restent sûrs sans lecture en cours', () async {
